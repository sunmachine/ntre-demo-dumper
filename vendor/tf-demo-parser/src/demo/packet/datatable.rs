    ...
    fn parse(stream: &mut Stream, _state: &ParserState) -> Result<Self> {
        ...
        for _ in 0..prop_count {
            ...
            if prop.flags.contains(SendPropFlag::InsideArray) {
                // NT;RE patch: the engine permits ChangesOften on array element
                // props (it only affects server-side prop priority), and NT;RE's
                // DT_NEO_Player.m_rfAttackersAccumlator uses it; only two
                // consecutive element props are actually malformed.
